...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    void api.get<{ data: { providerStatus: ProviderStatus } }>("/settings", { signal: controller.signal })
      .then((response) => setProviderStatus(response.data.data.providerStatus))
      .catch(() => undefined);
  ...
  return <Layout><div className="mx-auto max-w-6xl"><p className="text-sm font-semibold uppercase tracking-[.18em] text-cyan-300">Configuration</p><h1 className="mt-2 text-4xl font-bold">Settings</h1><p className="mt-3 text-slate-400">Manage identity, provider status, limits, privacy, and campaign defaults. Provider secrets are never displayed.</p>
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6"><h2 className="flex items-center gap-2 text-lg font-semibold"><ShieldCheck size={19} className="text-cyan-300" /> Provider status</h2><div className="mt-5 space-y-3"><div className="rounded-xl bg-slate-950 p-4"><p className="flex items-center gap-2 font-medium"><Search size={17} /> Research</p><p className="mt-2 text-sm text-slate-400">{providerStatus?.research.configured ? `${providerStatus.research.provider} configured` : providerStatus?.research.message ?? "Checking configuration…"}</p></div><div className="rounded-xl bg-slate-950 p-4"><p className="flex items-center gap-2 font-medium"><Mail size={17} /> Email delivery</p><p className="mt-2 text-sm text-slate-400">{providerStatus?.email.configured ? `${providerStatus.email.provider} configured` : "Not configured"} · outbound {providerStatus?.email.outboundEnabled ? "enabled" : "disabled"} · {providerStatus?.email.deliveryMode ?? "disabled"} mode</p></div><p className="text-xs text-slate-500">Keys remain server-side. Only configured/not configured state is returned.</p></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm text-slate-300">AI provider<select value={provider} onChange={(event) => setProvider(event.target.value as AiProvider)} className={fieldClass}><option value="MOCK">Mock AI</option><option value="GROQ">Groq</option></select></label><label className="text-sm text-slate-300">Theme<select value={theme} onChange={(event) => setTheme(event.target.value as Theme)} className={fieldClass}><option value="DARK">Dark</option><option value="LIGHT">Light</option><option value="SYSTEM">System</option></select></label></div></section>
